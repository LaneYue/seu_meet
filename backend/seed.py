...
achievements_data = [
    # S 四牌楼
    ("SL01","老图钉子户","S","L","连续 7 天在老图书馆签到自习","老图的门卫大爷已经把你的脸加进了白名单，进出不用刷卡了",1),
    ("SL02","大礼堂前晨读者","S","L","清晨 6:30 前在大礼堂前晨读 5 次","你比大礼堂的保洁阿姨起得都早，连鸽子都来听你背单词",2),
    ("SL03","涌泉池畔思考者","S","L","在涌泉池边坐满 30 分钟 x 10 次","涌泉池的水都认识你了，连金鱼都学会点头",3),
    ("SI01","蓁巷搭子","S","I","和 1 个搭子一起走完蓁巷全段","蓁巷的猫都认识你俩了，下雨天还会等你",4),
    ("SI02","梧桐树下对话","S","I","在梧桐树下和陌生人完成 1 次 5 分钟以上对话","一片梧桐叶落在你俩中间，民国文学的剧本有了",5),
    ("SI03","四牌楼合影狂魔","S","I","集齐 10 个四牌楼隐藏机位打卡","你的朋友圈最近 30 张都是四牌楼，朋友以为你迷路了",6),
    ("SM01","四牌楼十日环游","S","M","连续 10 天绕四牌楼走一圈","你的微信步数成了宿舍 daily TOP 1，室友以为你兼职跑腿",7),
    ("SM02","四牌楼夜猫","S","M","凌晨 2 点后从四牌楼大门回宿舍","门卫大爷心疼地塞给你一袋零食，劝你搬回学校住",8),
    ("SM03","健雄院夜行者","S","M","深夜 11 点后路过健雄院 5 次","吴健雄先生在天上看着你，微微点头：你比当年的她还卷",9),
    ("SF01","蓁巷锅贴侠","S","F","在蓁巷吃遍 5 家锅贴店","老板们已经能闭着眼做出你的那份，还附赠一杯醋",10),
    ("SF02","南门扫街王","S","F","四牌楼南门小吃街打卡 8 家店","你的大众点评 LV 等级在东大无人能敌，连新店老板都先敬你三分",11),
    ("SF03","四牌楼咖啡鉴赏家","S","F","在四牌楼喝过 5 家不同的咖啡馆","你的舌头能品出哪杯是周末特调，咖啡师开始怕你了",12),
    ("SH01","大礼堂见证者","S","H","参加 1 次大礼堂内举办的活动","你见证了百年大礼堂的第 N 次掌声，门票钱都值回来了",13),
    ("SH02","吴健雄追光者","S","H","参观吴健雄纪念馆并提交 1 段感悟","物理学奖离你远了点，但你的脑洞里种下了一颗原子核",14),
    ("SH03","中央大学溯源者","S","H","解锁 5 个中央大学旧址打卡点","1902 的风从老照片里吹出来，你闻到了中央大学食堂的红烧肉味",15),
    # J 九龙湖
    ("JL01","图书馆占座王","J","L","学期内累计占座 30 天","你的书包和保温杯在图书馆有了固定工位，清洁阿姨都认识",16),
    ("JL02","通宵自习幸存者","J","L","在通宵自习室撑到天亮","太阳升起来时你眼睛还亮着，靠的不是咖啡，是不想输",17),
    ("JL03","橘园考研陪跑员","J","L","连续 100 天进入图书馆复习","Day 1 你在，Day 100 你还在，连图书馆的空调都为你单独开过夜档",18),
    ("JI01","橘园搭子","J","I","找到 3 个橘园固定饭搭","橘园 4 个窗口的阿姨都认识你们仨，问你今天几个人",19),
    ("JI02","梅园球友","J","I","在梅园球场认识 5 个球友","你的篮球鞋和梅园的塑胶地摩擦出了火花，被称梅园三步上篮王",20),
    ("JI03","桃园饭搭","J","I","桃园食堂和不同人吃饭 10 次","桃园的米线师傅看你换搭档的速度，比你点单的速度还快",21),
    ("JM01","九龙湖环湖者","J","M","完成 1 次九龙湖环湖跑","你跑完了整个湖，校车司机都问你怎么不坐车",22),
    ("JM02","将军山远足","J","M","登顶将军山 1 次","你在山顶拍的照片发到群里，室友以为你去了川西",23),
    ("JM03","校车通勤王","J","M","一学期内乘坐九龙湖到四牌楼校车 30 次","校车司机一看到你就喊四牌楼是吧第三个位子给你留着",24),
    ("JF01","食堂全勤奖","J","F","把 4 个食堂全吃一遍","你解锁了橘园梅园桃园楠园的全图鉴，食堂阿姨集体给你鼓掌",25),
    ("JF02","殷巷探店王","J","F","殷巷商业街打卡 10 家店","你的殷巷足迹覆盖了整条街，连街角的奶茶店都送你终身 8 折",26),
    ("JF03","橘园夜宵王","J","F","橘园夜宵档口吃过 15 次","凌晨的橘园你比保安还熟，夜宵摊老板给你留了专属座位",27),
    ("JH01","校歌一遍过","J","H","一字不差唱完校歌","你 K 歌新曲库 +1 东南大学校歌，朋友让你别再唱了",28),
    ("JH02","止于至善","J","H","在校园内集齐止于至善 4 字拓印","这 4 个字你临摹了 100 遍，连书法协会都想收你",29),
    ("JH03","东大人认证","J","H","完成新生入学典礼后获得","拿到学生证那一刻，你的人生履历上多了东大两个字，简历正式开光",30),
    # D 丁家桥
    ("DL01","丁家桥学神","D","L","期末成绩单 GPA 满绩","你的 GPA 让教务处打印机的墨水都紧张，全校第一排不上号至少是个传说",31),
    ("DL02","基础自习室钉子户","D","L","在丁家桥基础自习室累计 50 小时","自习室的椅子被你的体温捂热了 50 小时，管理员说这是它最辉煌的 50 小时",32),
    ("DI01","丁家桥老友","D","I","在丁家桥认识 3 个长期搭子","丁家桥的食堂咖啡馆成了你第二个客厅，老板以为你住附近",33),
    ("DM01","玄武湖晨跑","D","M","完成 1 次玄武湖晨跑","你在玄武湖边看日出，看湖的老人对你说年轻人真有精神",34),
    ("DF01","丁家桥食堂王","D","F","食堂全菜系打卡","你吃遍食堂所有窗口，连新来的厨师都来问你哪道最稳定",35),
    ("DF02","小红山早茶客","D","F","小红山附近早茶店打卡 5 次","早茶店老板已经把加蛋加肠记成你的默认配置",36),
    ("DH01","医学生誓言","D","H","重温医学生誓言并录音","你的誓言被自己听到第二遍，连路过的小狗都肃然起敬",37),
    ("DH02","丁家桥老校友","D","H","听 1 位丁家桥老校友讲一次校史","你坐在老校友对面，听他讲完 50 年，听懂了医德两个字怎么写",38),
    # C 跨校区
    ("CL01","三校区图书馆全打卡","C","L","三个校区图书馆都签到","你的图书馆借阅卡上 3 个校区的章集齐，图书管理员集体为你打 call",39),
    ("CL02","校车蹭课王","C","L","跨校区蹭课 5 次","你的课表上出现了 3 个校区的教室，老师以为你是交换生",40),
    ("CI01","三校区搭子","C","I","在 3 个校区都有搭子","你的微信定位一会儿四牌楼一会儿九龙湖一会儿丁家桥，警察叔叔表示关注",41),
    ("CI02","校车情缘","C","I","在校车上认识 1 个新朋友","一次校车坐出爱情故事，对方成了你的毕生搭子",42),
    ("CI03","搭子宇宙中心","C","I","校内搭子数达到 20 人","你的搭子比某些人的朋友还多，他们私下叫你东大社交天花板",43),
    ("CM01","南京地铁探索者","C","M","乘坐南京地铁 10 条线路","你的南京地铁卡刷成了金卡，进站闸机都对你点头",44),
    ("CM02","三校区 city walk","C","M","一天内走完 3 个校区","你的微信步数当天破 5 万，朋友圈直接爆了，室友以为你出事了",45),
    ("CM03","东大边界探索者","C","M","解锁东大所有隐藏边界","你比保安还清楚学校的每个角落，保安队长想挖你",46),
    ("CF01","南京吃货","C","F","吃遍南京 5 大老字号","你的大众点评有 200+ 收藏，卫岗牛奶都知道你的名字",47),
    ("CF02","三校区食堂王者","C","F","三个校区食堂全部集齐徽章","你的饭卡在三个校区都成了 VIP 客户，食堂阿姨叫得出你姓名",48),
    ("CH01","东南精神继承者","C","H","解锁所有 SH+JH+DH 徽章","14 个文化类徽章发光环绕你，你已经站在了止于至善的门口",49),
    ("CH02","东大老照片收藏家","C","H","收集 10 张东大老照片","你的相册里有一整个东大百年史，老教授们看着你发来点赞",50),
]

existing_codes = {a.code for a in db.query(Achievement).all()}
new_count = 0
for code, name, campus, category, cond, humor, order in achievements_data:
    if code not in existing_codes:
        db.add(Achievement(
            code=code, name=name, campus=campus, category=category,
            conditionDesc=cond, humorDesc=humor,
            imagePath=f"/achievements/{code}.png",
            sortOrder=order,
        ))
        new_count += 1

db.commit()
print(f"  ✅ 写入了 {new_count} 条成就数据（跳过已存在 {len(existing_codes)} 条）")
...
if first_user and db.query(PlazaPost).count() == 0:
    plaza_posts = [
        {"title": "今晚图书馆自习搭子", "category": "study", "time": "今天 19:00-22:00",
         "place": "九龙湖图书馆 研习区", "capacity": 6, "note": "一起专注学习，互相监督，效率翻倍。"},
        {"title": "四牌楼文化同行路线", "category": "life", "time": "本周六 14:00-17:00",
         "place": "四牌楼校区", "capacity": 8, "note": "一起逛校园历史建筑，拍照打卡。"},
        {"title": "梅园篮球组队", "category": "sport", "time": "每周三 18:00-20:00",
         "place": "九龙湖梅园球场", "capacity": 10, "note": "缺几个人凑队，欢迎任何水平。"},
    ]
    for p in plaza_posts:
        db.add(PlazaPost(authorId=first_user.id, **p, joinedCount=1))
    db.commit()
    print(f"  ✅ 写入了 {len(plaza_posts)} 条广场种子帖子")
else:
    print("  ⏭️  广场帖子已存在，跳过")
