...
export function MyProfileEditPage() {
  ...
  const previewProfile = useMemo(() => ({
    ...myProfile,
    post,
    interests: interests.split(/[、,，]/).map((item) => item.trim()).filter(Boolean).slice(0, 6),
    ...
    <div className="app-screen detail-screen">
      ...
      <div className="scroll-page profile-editor-page">
        <header className="simple-page-top">
          ...
          <button className="icon-button" aria-label="保存装点">
            <Save size={19} />
          </button>
        </header>
        ...
        <section className="glass-card editor-panel">
          <label>
            <span><Sparkles size={16} />主页文案</span>
            <textarea value={post} onChange={(event) => setPost(event.target.value)} rows={4} />
          </label>
          <label>
            <span><Sparkles size={16} />兴趣标签</span>
          ...
          <div>
            ...
            <div className="theme-photo-row">
              {themePhotos.map((item) => (
                <button
                  className={photo === item.url ? "selected" : ""}
                  key={item.url}
        ...
        <Link className="full-width-action" to="/me">完成预览</Link>
      </div>
    </div>
  )
}
